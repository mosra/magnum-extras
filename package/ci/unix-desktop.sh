...
# Verify that the files generated by the theme generator are up-to-date. If
# not, the diff will show non-empty output and diff-index exits with a non-zero
# code. As a side-effect this also collects code coverage for the generator,
# which is important as there shouldn't be any dead code.
#
# Relies on __PRETTY_FUNCTION__ showing enum names, which is only since GCC 9+,
# so it's not run always
if [ "$UI_CHECK_GENERATE_THEME" != "OFF" ]; then
    ninja MagnumUi-generate-theme
